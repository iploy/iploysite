<?php
	...
	// PROCESS EDIT FORM
	if($_GET['subaction']=='edit'&$_POST){
		$affiliateUpdate = new affiliate ;
		$affiliateUpdate->setAffiliateUserId($_SESSION['user_id']) ;
		...
		if($affiliateUpdate->usernameIsUnique($_POST['username'],$_SESSION['affiliate_username'])==false){
		...
		if(sizeof($screen_message_array)==0){
			...
			if($affiliateUpdate->updateAffiliate(true)){
				header('Location: home.php?action=affiliates&update=good') ;
				exit() ;
			} else {
			...
			// set the form from the new post array for re-editing if failed
			$populateFormFromSessions = false ;
			$screen_message = '<b>Your affiliate account could not be created due to the following errors</b>:' ;
			$screen_message_type = 'error' ;
		}
		
	} else {
		// build the form data array from the sessions
		$affiliateInfo['username'] = $_SESSION['affiliate_username'] ;
		$affiliateInfo['payment_name'] = $_SESSION['affiliate_payment_name'] ;
		$affiliateInfo['payment_address_1'] = $_SESSION['affiliate_payment_address_1'] ;
		$affiliateInfo['payment_address_2'] = $_SESSION['affiliate_payment_address_2'] ;
		$affiliateInfo['payment_address_town'] = $_SESSION['affiliate_payment_address_town'] ;
		$affiliateInfo['payment_address_county'] = $_SESSION['affiliate_payment_address_county'] ;
		$affiliateInfo['payment_address_postcode'] = $_SESSION['affiliate_payment_address_postcode'] ;
	}
